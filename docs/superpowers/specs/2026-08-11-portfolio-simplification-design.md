...
1. **Keep the keyframes, retokenize their timing.** shadcn ships `accordion-up` / `accordion-down` `@keyframes` driven by `--radix-accordion-content-height`, with a hardcoded `0.2s ease-out`. Change only the timing, to `var(--duration-med) var(--ease-out)`. Do **not** convert to a CSS transition: Radix Accordion's `Content` mounts through `@radix-ui/react-presence`, whose `usePresence` detects exit only via `animationName` and `animation*` events with no `transitionend` handling, so a transition-based panel snaps shut with no animation. shadcn ships keyframes because the primitive requires them.
