...
const Avatar = ({ isDarkMode }: { isDarkMode: boolean }) => {
  ...
  const themeColors: ThemeColors = useMemo(
    () => ({
      background: isDarkMode ? "hsl(0, 0%, 18%)" : "#a9ddf3",
      bodyStroke: isDarkMode ? "hsl(45, 49%, 80%)" : "#3A5E77",
      bodyFill: isDarkMode ? "hsl(0, 0%, 25%)" : "#FFFFFF",
      bodyBottom: isDarkMode ? "hsl(0, 0%, 30%)" : "#DDF1FA",
      earFill: isDarkMode ? "hsl(0, 0%, 30%)" : "#ddf1fa",
      earStroke: isDarkMode ? "hsl(45, 49%, 80%)" : "#3a5e77",
      hairFill: isDarkMode ? "hsl(0, 0%, 25%)" : "#FFFFFF",
      hairStroke: isDarkMode ? "hsl(45, 49%, 80%)" : "#3A5E77",
      face: isDarkMode ? "hsl(0, 0%, 30%)" : "#DDF1FA",
      eyeFill: isDarkMode ? "hsl(45, 49%, 80%)" : "#3a5e77",
      mouthFill: isDarkMode ? "hsl(45, 49%, 60%)" : "#617E92",
      mouthStroke: isDarkMode ? "hsl(45, 49%, 80%)" : "#3A5E77",
      noseFill: isDarkMode ? "hsl(45, 49%, 80%)" : "#3a5e77",
    }),
    [isDarkMode]
  );
  ...
  const calculateEyePosition = useCallback(
    (eyeCenterX: number, eyeCenterY: number): EyePosition => {
      ...
      const deltaX = mousePos.x - actualEyeCenterX;
      const deltaY = mousePos.y - actualEyeCenterY;
  ...
  useEffect(() => {
    setEyePositions({
      left: calculateEyePosition(85.5, 78.5),
      right: calculateEyePosition(114.5, 78.5),
    });
  }, [calculateEyePosition]);

  const handleAvatarHover = useCallback((hovering: boolean) => {
    setIsHovering(hovering);
    if (hovering) {
      setEyePositions({ left: { x: 0, y: 0 }, right: { x: 0, y: 0 } });
    }
  }, []);

  // Memoized eye state for performance
  const eyeState = useMemo(
    () => ({
      radius: isBlinking ? 0.3 : isHovering ? 3 : 3.5,
      highlightRadius: isBlinking || isHovering ? 0 : 1,
    }),
    [isBlinking, isHovering]
  );

  // Reusable ear component
  const Ear = ({
    side,
    cx,
    cy,
    ...
    <g className={`ear${side === "left" ? "L" : "R"}`}>
      <g className="outerEar">
        <circle
          fill={themeColors.earFill}
          stroke={themeColors.earStroke}
          strokeWidth="2.5"
          cx={cx}
          cy={cy}
          r="11.5"
        ...
        <path
          ...
          d={
            side === "left"
              ? "M46.3 78.9c-2.3 0-4.1 1.9-4.1 4.1 0 2.3 1.9 4.1 4.1 4.1"
              : "M153.7,78.9 c2.3,0,4.1,1.9,4.1,4.1c0,2.3-1.9,4.1-4.1,4.1"
          }
        />
      </g>
      <g className="earHair">
        <rect
          x={side === "left" ? "51" : "134"}
          y="64"
          fill={themeColors.hairFill}
          width="15"
          height="35"
        />
        <path
          fill={themeColors.hairFill}
          stroke={themeColors.hairStroke}
          strokeWidth="2.5"
          strokeLinecap="round"
          strokeLinejoin="round"
          d={
            side === "left"
              ? "M53.4 62.8C48.5 67.4 45 72.2 42.8 77c3.4-.1 6.8-.1 10.1.1-4 3.7-6.8 7.6-8.2 11.6 2.1 0 4.2 0 6.3.2-2.6 4.1-3.8 8.3-3.7 12.5 1.2-.7 3.4-1.4 5.2-1.9"
              : "M146.6,62.8 c4.9,4.6,8.4,9.4,10.6,14.2c-3.4-0.1-6.8-0.1-10.1,0.1c4,3.7,6.8,7.6,8.2,11.6c-2.1,0-4.2,0-6.3,0.2c2.6,4.1,3.8,8.3,3.7,12.5 c-1.2-0.7-3.4-1.4-5.2-1.9"
          }
        />
      </g>
    </g>
  ...
  return (
    ...
    >
      <svg
        ref={svgRef}
        className="w-full h-full cursor-pointer transition-colors duration-fast"
      ...
      >
        ...
        {/* Background circle */}
        <circle cx="100" cy="100" r="100" fill={themeColors.background} />

        {/* Body */}
        <g className="body">
          <path
            stroke={themeColors.bodyStroke}
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
            fill={themeColors.bodyFill}
        ...
        <g className="mouth">
          ...
          <path
            fill="none"
            stroke={themeColors.mouthStroke}
            strokeWidth="2.5"
            strokeLinejoin="round"
            d="M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z"
          />
