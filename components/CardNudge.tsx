 ...
 *
 * No surface. It was a `Bento` card, a bordered panel on `bg-card` that
 * swapped to `bg-elevated` on hover, and inside a section that is already a
 * headed block of contact details it read as a second panel competing with
 * the first. Stripped to what it says: a chip, two lines, the fan, the link.
 * The hover colour swap went with the background it was swapping, since
 * there is nothing left to tint. The fan still opens, the chip still tilts
 * and the arrow still slides, which is the motion that was worth having.
 *
 * The eyebrow above the title ("5 editions - 2 dice x 3 rolls") is gone
 * too. It priced the mechanic before saying what was on offer, and the
 * numbers mean nothing to someone who has not seen a card yet.
 *
 ...
 *
 * Sized from the reference's tuned values rather than the app's
 ...
 * chip's tilt, the arrow's slide) rides the app's own `duration-med` /
 * `ease-out` tokens.
 *
 ...
 * either. With the panel gone there is no longer a rounded, overflow-hidden
 * border for it to be clipped by, so the spread is bounded by the reserved
 * box alone.
 *
...
export default function CardNudge({ className }: { className?: string }) {
  return (
    <div className={cn(className)}>
      <Link
        href="/card"
        className="group flex flex-col gap-5 md:flex-row md:items-center md:justify-between md:gap-6"
      >
        <div className="flex items-start gap-3.5">
          {/* The chip is the only filled shape left, and now the thing that
              marks this as an offer rather than another paragraph. The tint
              (`bg-elevated` plus a ring) rather than a bare border: against
              the page in dark theme, an unfilled chip reads as an outline
              drawn on nothing. */}
          <div
            aria-hidden="true"
