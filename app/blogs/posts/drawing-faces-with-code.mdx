...
So the trick is that <Marker>you do not draw a line at all</Marker>. You build a filled shape and vary its width along the path:
...
<Marker>None of this code knows what a face is.</Marker> The same functions draw hats, sunglasses, a cowboy brim and the handwritten name under each portrait.
...
That property is usually an annoyance. <Marker>Here it is the entire feature.</Marker>
...
The bias was working perfectly. The real fault was that <Marker>pale hair rendered too faint to see</Marker>, so long-haired portraits read as bald. I would have spent an evening rewriting the correct half.
...
Photocopy a postcard onto a poster and the lines get fat and clumsy. A drawing actually made at poster size has the same thin pencil lines the postcard had, because <Marker>the pencil tip never changed</Marker>. There are simply more of them, carrying more detail.
...
Generated art has the same shape, and it is worse, because <Marker>failures hide</Marker>. Eight portraits appear, one is broken, you refresh and it is gone. I spent days chasing artefacts I could not reproduce, guessing at causes, and shipping fixes that changed nothing.
...
<Marker>Keep deciding separate from drawing.</Marker> You get to measure what your generator does without rendering anything, and every fix stays local.

<Marker>Build the contact sheet before you think you need it.</Marker> Every hour I spent staring at random output was an hour I could have spent looking at all the cases at once.
