...
const SpiderAnimation = () => {
  ...
  const handleMouseMove = useCallback(
    (e: any) => {
      ...
      if (distance < 100) {
        // If mouse is within 100px
        const escapeX = position.x - dx;
        const escapeY = position.y - dy;
        setPosition({ x: escapeX, y: escapeY });

        // Update rotation when escaping
  ...
  useEffect(() => {
    const interval = setInterval(moveSpider, 3000);
