...
- **All visitor-card drawing** goes through `lib/card/`. The pure modules (`seed`, `issues`, `types`) carry vitest tests and must stay free of DOM access; the drawing modules (`sticker`, `ticket`, and the vendored `engine`) take a canvas context as a parameter and must never hold module-level mutable state, because the issue gallery renders six canvases on one page. `drawTicket` draws every size: preview, export and thumbnail. Never add a second drawing routine.
...
npm test         # vitest run (lib/card's pure modules: seed, issues, types)
```
