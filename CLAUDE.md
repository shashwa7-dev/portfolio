...
- **The measure lives in one place**, `--measure` in `app/globals.css`. `Container width="reading"` and `Rails` both read it. Never write `760px` into a component: the rails and the column they describe have to move together. `--gutter` derives from it and positions the band's ink tick on the left rail. There is no gutter fill: dots were tried on both sides of every band and removed, so a band is its rules, its label and its tick.
