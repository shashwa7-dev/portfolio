...
- **`components/card/dice/`** holds the dice-roll UI: `useDiceRoll` (the shared roll state, the three fixes to its throw/settle/reset lifecycle, and the pill's click contract), `RollPill` (the shared button, fill bar and caption, driven by `useDiceRoll`'s own return value), `Pips` (the flat die-face icon), and the two skins `CubeDice` and `TossDice`. `DiceRoller.tsx` picks a skin via the `?dice=cube` / `?dice=toss` query param and exports `SkinProps`, the prop shape both skins take. The `web-haptics` dependency drives the throw's press, landing and completion taps; `safeHaptic` in `useDiceRoll.ts` makes sure a missing implementation, an unsupported browser, or a declined permission never interrupts a throw.
